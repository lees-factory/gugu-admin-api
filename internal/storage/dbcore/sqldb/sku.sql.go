...
const countSKUsByProductID = `-- name: CountSKUsByProductID :one
SELECT COUNT(*) FROM gugu.sku WHERE product_id = $1
`

func (q *Queries) CountSKUsByProductID(ctx context.Context, productID string) (int64, error) {
	row := q.db.QueryRowContext(ctx, countSKUsByProductID, productID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
